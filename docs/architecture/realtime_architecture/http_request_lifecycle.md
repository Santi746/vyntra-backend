...
> [!NOTE]
> **Lo obvio:** El HTTP y el WebSocket son dos canales distintos que conviven. El HTTP responde al usuario que escribe, el WebSocket notifica a los demás.

> [!IMPORTANT]
> **Lo no tan obvio:** El Controller **nunca** llama a Reverb directamente. Despacha un `Event` con `ShouldBroadcast + ShouldQueue`, lo que mete el evento en una **cola de Redis**. Un proceso aparte (`queue:work`) lo saca y se lo pasa a Reverb. Esto desacopla la respuesta HTTP del envío WebSocket.

> [!NOTE]
> El viaje completo del evento broadcast (HTTP → Controller → Redis → Worker → Reverb → Cliente) está documentado en [`./realtime_data_lifecycle.md`](./realtime_data_lifecycle.md). La primera mitad (HTTP) la ves en este documento; la segunda mitad (WebSocket) la ves en el otro.
